...
class WaEquipSession(models.Model):
    ...
    step = fields.Selection(
        [("await_items", "Awaiting item list"),
         ...
         ("cs_job", "Crew select — awaiting job pick"),
         ...
         ("qc_pick", "Quote client — pick existing / new (WA-12)"),
         ("qc_kind", "Quote client — company or individual (WA-12)"),
         ("qc_name", "Quote client — awaiting client name (WA-12)"),
         ("qc_dupe", "Quote client — near-duplicate confirm (WA-12)"),
         ("qc_contact", "Quote client — awaiting contact person (WA-12)"),
         ("qc_phone", "Quote client — awaiting phone (WA-12)"),
         ("qc_email", "Quote client — awaiting email (WA-12)"),
    ...
    @api.model
    def _start_fb(self, phone_e164, user, step, buf):
        """WA-10 — open (or rebind) the single session into a feedback step:
        'fb_pull' (DICT buffer {event_ids:[...]} -- the role-aware wrapped-
        event list the staffer picks from) or 'fb_notes' (DICT buffer
        {event_job_id, role, fb_id} -- the row a free-text note updates after
        a sentiment tap). Same rebind-the-unique-row reasoning as _start."""
        sess = self.sudo().with_context(active_test=False).search(
            [("phone_number", "=", phone_e164)], limit=1)
        vals = {
            "user_id": user.id if user else False,
